...
def update_book(book_id, title, author, genre, year, pages, isbn, price=None):
    try:
        with db_cursor() as cursor:
            if price is not None:
                sql = """
                    UPDATE books SET
                    title = %s, author = %s, genre = %s, year = %s, pages = %s, isbn = %s, price = %s
                    WHERE book_id = %s
                """
                cursor.execute(sql, (title, author, genre, year, pages, isbn, price, book_id))
            else:
                sql = """
                    UPDATE books SET
...
def return_book(book_id, return_date, user_id=None):
    try:
        with db_cursor() as cursor:
            if user_id is not None:
                cursor.execute(
                    """
                    UPDATE borrowed_books SET return_date = %s
...
def create_book(title, author, genre, year, pages, isbn, price=0):
    try:
        with db_cursor() as cursor:
            sql = """
            ...
            """
            cursor.execute(sql, (title, author, genre, year, pages, isbn, price))
